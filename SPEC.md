# swux v1 Specification

## Purpose

`swux` is a small Rust CLI for tmux session selection and creation.
...
`swux` is intentionally a thin orchestration layer over existing tools. It must stay small, predictable, and easy to debug.
...
Create a starter config at `~/.config/swux/config.toml` if it does not already exist.

## Runtime Dependencies

`swux` invokes the following external programs:
...
If the resolved session name already exists, `swux` must reuse it. v1 does not add numeric suffixes automatically.
...
`swux` must work without a config file.
...
hint: define [templates.rust] in ~/.config/swux/config.toml
```
...
- starter config generated by `swux init`
...
- `swux connect <path>` works without templates
...
4. How much shell quoting logic is owned by `swux` for `send-keys`. Recommendation: keep commands as raw strings sent to tmux, avoid inventing a shell mini-language.
