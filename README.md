...
With Nix, from nixpkgs:

```bash
nix profile install nixpkgs#smux
```

The nixpkgs package was added in [NixOS/nixpkgs#505348](https://github.com/NixOS/nixpkgs/pull/505348). If your local registry or channel has not caught up yet, install from the project flake:
...
Templates shine because you rarely pick one by hand. Each template declares what it matches — marker files via `match` (exact names or globs like `nuxt.config.*`) and/or `package.json` dependencies via `match_dependencies` (for the likes of `react` and `vue`, which have no config file). When you open a folder, smux applies the matching template; when several match, `priority` breaks the tie (so `next` beats `react`, `nuxt` beats `vue`). `smux init` ships templates for the common languages (Rust, Go, Python, Ruby, Java, Node) so detection works out of the box, and the [templates guide](./docs/templates.md) has copy-paste framework templates (React, Vue, Svelte, Angular, Astro, Next, Nuxt) that auto-detect the moment you drop them in. There's no built-in list to work around: adding a `match` to a template *is* how you extend it. If nothing matches but you have several templates, smux pops up a quick chooser. Teach your templates what to match, and folders open with the right workspace on their own.
...
on_create = "docker compose up -d"  # runs once in the session root before the session is created
windows = [
