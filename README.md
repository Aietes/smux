...
Templates shine because you rarely pick one by hand. Each template declares what it matches — marker files via `match` (exact names or globs like `nuxt.config.*`) and/or `package.json` dependencies via `match_dependencies` (for the likes of `react` and `vue`, which have no config file). When you open a folder, smux applies the matching template; when several match, `priority` breaks the tie (so `next` beats `react`, `nuxt` beats `vue`). The templates `smux init` ships already cover the common types — Rust, Go, Python, Ruby, Java, Node, plus React, Vue, Svelte, Angular, Astro, Next, and Nuxt — so detection works out of the box, and there's no built-in list to work around: adding a `match` to a template *is* how you extend it. If nothing matches but you have several templates, smux pops up a quick chooser. Teach your templates what to match, and folders open with the right workspace on their own.
