...
- `smux init` now scaffolds templates for common project types — `rust`, `node`, `go`, `python`, `ruby`, `java`, plus `react`, `vue`, `svelte`, `angular`, `astro`, `next`, and `nuxt` — each carrying its own markers, so opening a recognized folder applies the right layout out of the box; the starter config leaves `default_template` unset so smart selection stays on
